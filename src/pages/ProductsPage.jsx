...
const ProductsPage = () => {
  ...
  const fetchProducts = useCallback(async (term) => {
    ...
    try {
      const result = await window.api.getProducts(term);
  ...
  }, [searchTerm, fetchProducts]); // Re-executa quando searchTerm ou fetchProducts muda

    // Função para abrir o modal (será usada depois)
    const handleOpenModal = () => {
        console.log("Abrir modal de criação...");
        setIsModalOpen(true); // Descomentar quando o modal existir
